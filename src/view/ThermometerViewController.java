...
public class ThermometerViewController
{
  ...
  public void init(ViewHandler viewHandler, ThermometerViewModel viewModel, Region root)
  {
    ...
    Bindings.bindBidirectional(fistThermometerLabel.textProperty(), viewModel.getFirstIndoorProperty(), new StringIntegerConverter(0));
    Bindings.bindBidirectional(secondThermometerLabel.textProperty(), viewModel.getSecondIndoorProperty(), new StringIntegerConverter(0));
    Bindings.bindBidirectional(outsideThermometerLabel.textProperty(), viewModel.getOutdoorProperty(), new StringIntegerConverter(0));

    Bindings.bindBidirectional(criticalHighLabel.textProperty(), viewModel.getHighValueProperty(), new StringIntegerConverter(0));
    Bindings.bindBidirectional(criticalLowLabel.textProperty(), viewModel.getLowValueProperty(), new StringIntegerConverter(0));

    Bindings.bindBidirectional(lowValue.textProperty(), viewModel.setCriticalLowTemperature(), new StringIntegerConverter(0));
    Bindings.bindBidirectional(highValue.textProperty(), viewModel.setCriticalHighTemperature(), new StringIntegerConverter(0));
...
viewModel.getAll();
  ...
  {
    if( Double.parseDouble(highValue.getText())<= 40 && Double.parseDouble(lowValue.getText()) >= 10)
    {
      return false;
    }
    ...
    if(Double.parseDouble(highValue.getText()) <= 40)
    {
      lowValue.setText("10");
      alert.setHeaderText(
          "Minimum temperature can not be lower than 10");
    } else if (Double.parseDouble(lowValue.getText()) >= 10)
    {
      highValue.setText("40");
      alert.setHeaderText(
          "Maximum temperature can not be higher than 40");
    }
